...
const AppApprovalsHistogram = (): ReactElement | null => {
	...
	const lastThreeMonths: RangePickerPresetRange = [moment().startOf("month").subtract(3, "months"), moment().endOf("month")];
			...
			chart.legend.itemContainers.template.events.on("hit", ev => {
				...
				setTimeout(() => {
					chart.series.each(item =>
						item !== series ? item.hide() : item.show(),
					);
				}, 10);
			});
	...
	return (
		<BigCard title="App Approvals" headerChild={
			<RangePicker
