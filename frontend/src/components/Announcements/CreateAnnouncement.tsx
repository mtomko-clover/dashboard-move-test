...
import { CreateAnnouncementExclamationIcon, EditRow, EditInput} from "./Announcement.styles";
...
const CreateAnnouncement = (): ReactElement => {
    ...
    const mutation = gql`
        mutation CreateAnnouncement($text: String, $is_urgent: Boolean, $username: String) {
            createAnnouncement(text: $text, is_urgent: $is_urgent, username: $username) {
                message
            }
        }
    `;
    ...
    const handleisUrgentChange = (e: CheckboxChangeEvent): void => {
        setFormState((prevState) => ({ ...prevState, isUrgent: e.target.checked }));
    };
    ...
    return (
        <Modal
            ...
            destroyOnClose={true}>
            <EditRow>
                <Checkbox className="margin-after" checked={isUrgent}  onChange={handleisUrgentChange}><CreateAnnouncementExclamationIcon className="fas fa-exclamation"/>Urgent</Checkbox>
                <EditInput type="text" placeholder="Announcement" name="text" value={text} onChange={handleInputChange}/>
            </EditRow>
        </Modal>
    )
};
