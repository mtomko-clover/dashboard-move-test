...
def pull_most_popular_apps(db, dids_to_exclude, asids_to_include=None):
    """
    Returns:
        list of lists
    ...
    """
    query = """
        SELECT da.name, da.uuid, da.id, d.name, d.uuid, d.id, sum(if(ma.deleted_time IS NOT NULL, 0, 1)) AS C
...
def update_app_leaderboard(db, country_code):
    spreadsheet_title = "[DA-174] Most Popular Apps"
    first_party_ids = pull_first_party_ids(prod_us)
    
    # ALL APPS
    worksheet_title = "Most Popular Apps US (Paid & Free)"
    logger.debug("Pulling most popular apps in US...")
    most_pop_apps = pull_most_popular_apps(db=prod_us,
                                           dids_to_exclude=first_party_ids,
                                           asids_to_include=pull_app_subscription_ids(prod_us, country_code))
    logger.info("Pulled most popular apps in US.")
    ...
    # FREE APPS
    worksheet_title = "Most Popular Apps US (Free)"
    logger.debug("Pulling most popular free apps in US...")
    most_pop_apps = pull_most_popular_apps(db=prod_us,
                                           dids_to_exclude=first_party_ids,
                                           asids_to_include=pull_app_subscription_ids(prod_us, country_code, "FREE"))
    logger.info("Pulled most popular free apps in US.")
    utils.clear_sheet_and_write(spreadsheet_title=spreadsheet_title,
                                worksheet_title=worksheet_title,
                                rows=most_pop_apps,
                                env=country_code)
    ...
    # PAID APPS
    worksheet_title = "Most Popular Apps US (Paid)"
    logger.debug("Pulling most popular paid apps in US...")
    most_pop_apps = pull_most_popular_apps(db=prod_us,
                                           dids_to_exclude=first_party_ids,
...
if __name__ == "__main__":
    ...
    try:
        ...
        country_code = "US"
        update_app_leaderboard(prod_us, country_code)
        prod_us.close()
