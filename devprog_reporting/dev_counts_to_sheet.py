...
from common_database_queries import count_third_party_devs_filter_on_timestamp_field
from environ import Environ, EnvironType

sys.path.append((os.path.abspath(os.path.join(os.path.dirname(__file__), '..')) + '/utilities'))
from configure_logger import configure_logger
from utils import get_sheet, write_row_to_sheet, phone_home, create_date_ranges
...
def create_row_third_party_devs_created_approved_between(environ, date_range):
    results = [environ.name_key, date_range[0].strftime("%Y-%m")]
    filter_fields = [Field("created_time", table=Table("developer")), Field("first_approval_time")]
    for filter_field in filter_fields:
        results.append(count_third_party_devs_filter_on_timestamp_field(environ=environ, date_range=date_range, filter_field=filter_field))
    return results

def write_monthly_count_devs_created_approved(environs, date_ranges=None):
    if not date_ranges:
    ...
    for date_range in date_ranges:
        for environ in environs:
            row = create_row_third_party_devs_created_approved_between(environ, date_range=date_range)
            sheet = get_sheet("[DA-170] App and Developer Counts", environ.name_key + "_devs_created_approved")
...
if __name__ == "__main__":
    ...
    try:
        write_monthly_count_devs_created_approved(environs)
