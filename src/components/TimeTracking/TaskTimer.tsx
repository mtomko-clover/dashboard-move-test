...
export class TaskTimer extends Component<TimerProps, State> {
  ...
  categoriesDropdown(): React.ReactElement {
    ...
    for (let category in categories) {
      const menuItem = (
        <Menu.Item key={category} className="dropdown" onClick={this.setCategory}>
          {category}
        </Menu.Item>
      );
  ...
  render(): React.ReactNode {
    let start =
      this.state.time === 0 ? (
        <TimerButton key="start" onClick={this.startTimer}>
          <i className="fas fa-play" />
        </TimerButton>
      ) : null;
    let stop =
      this.state.time === 0 || !this.state.isOn ? null : (
        <TimerButton key="stop" onClick={this.stopTimer}>
          <i className="fas fa-stop" />
        </TimerButton>
      );
    let resume =
      this.state.time === 0 || this.state.isOn ? null : (
        <TimerButton className="margin-after" key="resume" onClick={this.startTimer}>
          <i className="fas fa-play" />
        </TimerButton>
      );
    let reset =
      this.state.time === 0 || this.state.isOn ? null : (
        <TimerButton key="save" onClick={this.saveTime}>
      ...
      <TaskTimerContainer>
        {this.state.editing && (
          <NameInput
            ...
            onKeyPress={this.handleNameEntry}
          />
        )}
        {!this.state.editing && (
          <div onClick={this.editName} className="timer-title">
            {this.state.name}
          </div>
        )}
        <CategoryDropdown
        ...
        {this.state.subcategory != null && (
          <CategoryDropdown
            setCategory={this.setSubCategory}
            categories={this.state.subcategory}
            label="App Approval Step"
          />
        )}
